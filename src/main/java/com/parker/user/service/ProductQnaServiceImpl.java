...
public class ProductQnaServiceImpl implements ProductQnaService {
	...
	public void productQnaViewCnt(int productQna_number, HttpSession session) {
		...
		if (session.getAttribute("update_time_" + productQna_number) != null) {
			// 세션에서 읽어오기
			update_time = (long) session.getAttribute("update_time_" + productQna_number);
		}
		...
		if (current_time - update_time > 5 * 1000) {
			productQnaDAO.productQnaViewCnt(productQna_number);
